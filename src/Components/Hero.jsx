...
const Hero = () => {
  ...
  const deleteVideo = async (id) => {
    await axios.delete(`${BACKEND_URL}/Video/${id}`);
    setMsg(true);
    setTimeout(() => {
      setMsg(false);
    }, 3000);
  ...
  const handleUploadedImage = async (imageUrl, thumbnailUrl) => {
    ...
    try {
      const response = await axios.post(`${BACKEND_URL}/Video/`, {
        link: imageUrl,
      });
            ...
            <SwiperSlide key={index}>
              <div className="flex flex-col w-full justify-center">
                ...
                <div className="flex justify-center">
                  <RiDeleteBin6Fill
                    className="text-red-500 w-10 h-10 scale-100 hover:scale-125 cursor-pointer justify-center "
                    onClick={() => {
