...
const About = () => {
  ...
  async function handleupdate() {
    try {
      settoggle(!toggle);
      console.log(para);
      const { data } = await axios.put(`${BACKEND_URL}/AParagraph/line`, para);
    } catch (e) {
      console.log(e);
    }
  }
      ...
      <div className="text-center flex flex-col items-center text-gray-600 leading-7">
        ...
        <div className="hidden lg:flex flex-col items-center  ">
          <input
            type="text"
            readOnly={toggle}
            className=" text-center font2 mt-0 lg:mt-4 w-[320px] md:w-[550px] lg:w-[1200px] lg:text-[14px] tracking-wide text-gray-900"
            value={para[0]}
            onChange={(e) => handle(0, e)}
          />
          <input
            type="text"
            readOnly={toggle}
            className="text-center lg:w-[1100px] xl:w-[1100px] font2 text-gray-900"
            value={para[1]}
            onChange={(e) => handle(1, e)}
          />
          <input
            type="text"
            readOnly={toggle}
            className="text-center lg:w-[1000px] xl:w-[1000px] font2 text-gray-900"
            value={para[2]}
            onChange={(e) => handle(2, e)}
          />
          <input
            type="text"
            readOnly={toggle}
            className={` ${
              !toggle ? "border border-black" : null
            } text-center lg:w-[900px] xl:w-[900px] text-gray-900  font2`}
            value={para[3]}
            onChange={(e) => handle(3, e)}
          />
          <input
            type="text"
            readOnly={toggle}
            value={para[4]}
            className=" text-center lg:w-[800px] xl:w-[800px] text-gray-900  font2"
            onChange={(e) => handle(4, e)}
          />
          <input
            type="text"
            readOnly={toggle}
            value={para[5]}
            className=" text-center lg:w-[700px]  xl:w-[700px] text-gray-900 font2"
            onChange={(e) => handle(5, e)}
          />
          <input
            type="text"
            readOnly={toggle}
            value={para[6]}
            className=" text-center lg:w-[600px]  xl:w-[600px] text-gray-900 font2"
            onChange={(e) => handle(6, e)}
          />
          {!toggle ? (
            <button
        ...
        <div className="lg:hidden font2  text-justify px-5 pb-5 text-gray-900">
          ...
          located amidst the magnificent hills of Dehradun, Uttarakhand. With a
          vision to create an everlasting impact through our full-service
          production house. Under the able guidance of Mr. Tarun Singh Rawat,
          Vice Chairman- JSR Group, elder son of Mr. Jaswant Rawat. We have
          ...
          work, so we ensure that all our projects are completed with
          perfection. We know the value of time and hence ensure that we
          complete the project within the given time frame. We believe in
