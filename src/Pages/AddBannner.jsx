...
const AddBanner = () => {
  ...
  async function get() {
    try {
      const { data } = await axios.get(`${BACKEND_URL}/Banner/${type}`);
      setA(data);
    } catch (e) {
      console.log(e);
    }
  ...
  const addService = async () => {
    // console.log(a);
    const apiURL = `${BACKEND_URL}/Banner/${type}`;
    axios
      .put(apiURL, {
        img: imagesrc,
      })
  ...
  return (
    <>
      <div
        className="flex justify-center items-center p-10 bg-indigo-400 h-screen"
        style={{ fontFamily: "sans-serif" }}
      >
        <div className=" w-[30vw] p-6 shadow-lg bg-white rounded-md">
          <h1 className="text-3xl block text-center ">Update Banner</h1>
          <hr className="mt-3" />
