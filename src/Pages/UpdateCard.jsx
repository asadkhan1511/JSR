...
const UpdateCard = () => {
  ...
  async function getdata() {
    try {
      var { data } = await axios.get(`${BACKEND_URL}/RProject/${id}`);
      // console.log(data)
    } catch (error) {
      setType("U");
      var { data } = await axios.get(`${BACKEND_URL}/Upcoming/id/${id}`);
      // console.log(data)
    }
    // let {data}=await axios.get(`${BACKEND_URL}/RProject/${id}`);
    // if(data.message){
    //     setType("U");
    //     let {data}=await axios.get(`${BACKEND_URL}/Upcoming/id/${id}`);
    // }
  ...
  const updateCard = async () => {
    ...
    const apiURL =
      type == "R"
        ? `${BACKEND_URL}/RProject/${id}`
        : `${BACKEND_URL}/Upcoming/update/${id}`;
    axios
      .put(apiURL, a)
      .then(() => {
        navigate("/");
